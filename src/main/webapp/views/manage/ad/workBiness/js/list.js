...
function getData(data) {
	if(data != null && typeof data != "undefined") {
		$(obt).val(data.name);
		$("#responsibleUserName").val(data.name);
...
function initInputMask() {
	$("#workTime").inputmask("Regex", { regex: "\\d+\\.?\\d{0,2}" });
}
...
function getNodeHtml() {
	...
	html.push('<td><input type="text" name="responsibleUserName" id="responsibleUserName"  onclick="openDialog(this)" readonly class="input" value = "" ></td>');
