...
function getNodeHtml() {
	...
	html.push('<select id="type" name="type" class="form-control"  class="input" style="height:100%;width:100%;text-align-last:center;border: 0px">');
	html.push($("#addLevel").html());
	html.push('</select></td>');
	html.push('<td style="height:100%;"><input type="hidden" name="responsibleUserId" id="responsibleUserId" class="input" value = "" > <input type="text" name="responsibleUserName" id="responsibleUserName" onclick="openDialog(this)" readonly class="input" value = "" ></td>');
	...
	$.ajax({
		...
		success: function(data) {
			if(data.code == 1) {
				...
				bootstrapAlert("提示", "填写失败！", 400, null);
			}
		},
		error: function(data) {
...
function getFormJson() {
	...
	$("tr[name='node']").each(function(index, tr) {
		...
		if( !isNull(workDate) || (!isNull(startTime)&&startTime !=":00")
				...
				|| !isNull(responsibleUserId) || !isNull(content)) {
...
function checkForm(formData) {
	...
	if(formData["marketAttachsList"].length <= 0) {
		...
		$(formData["marketAttachsList"]).each(function(index, attach) {
			...
			if(attach["startTime"] == ":00") {
				text.push("开始时间不能为空！");
				return false;
			}
			if(attach["endTime"] == ":00") {
				text.push("结束时间不能为空！");
				return false;
			}
