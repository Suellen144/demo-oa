				...
				<div class="box box-primary tbspace">
					<form id="form1">
						...
						<table id="table1">
							<c:choose>
								<c:when test="${(map.task.name eq '提交申请' or map.task.name eq '部门经理') and map.business.userId eq sessionScope.user.id }">
									<thead>
										<c:choose>
											<c:when test="${map.business.status eq '3' }">
												<tr><th colspan="20">加班详情表</th></tr>
											</c:when>
											<c:otherwise>
												<tr><th colspan="20">加班申请表</th></tr>
											</c:otherwise>
										</c:choose>
									</thead>
									<tbody>
										<tr>
											...
											<td style="width:40%;line-height:inherit;">
										...
										<tr>
											...
											<td colspan="3"><input type="text" name="applyTime" value="<fmt:formatDate value="${map.business.applyTime}" pattern="yyyy-MM-dd" />" readonly></td>
										</tr>
										<tr>
											<td><span>加班起止时间</span></td>
											<td colspan="20" style="padding-right:30px;">
												自
												<input id="startTime" name="startTime" size="18" class="starttime" style="width:15%;" value="<fmt:formatDate value="${map.business.startTime}" pattern="yyyy-MM-dd HH:mm" />" readonly>
												至
											    <input id="endTime" name="endTime" size="18" class="endtime" style="width:15%;" value="<fmt:formatDate value="${map.business.endTime}" pattern="yyyy-MM-dd HH:mm" />" readonly>
											           共
											    <input id="days" name="days" value="${map.business.days }" style="width:3em;">
											    <label id="label_days">天</label>
											    <input id="hours" name="hours" value="<fmt:formatNumber value='${map.business.hours }' pattern='#.#' />" style="width:3em;">
											    <label id="label_hours">小时</label>
											</td>
										</tr>
										<tr>
											<td><span>加班事由</span></td>
											<td colspan="20"><textarea name="reason" rows="7" cols="70">${map.business.reason }</textarea></td>
										</tr>
									<%-- 	<tr>
											<td><span>附件</span></td>
								...
								<c:otherwise>
									...
									<tbody>
										...
										<tr>
											<td><span>提交时间</span></td>
											<td colspan="3"><input type="text" value="<fmt:formatDate value="${map.business.applyTime}" pattern="yyyy-MM-dd" />" readonly></td>
										</tr>
										<tr>
											<td><span>加班起止时间</span></td>
											<td colspan="20" style="padding-right:30px;">
												自
												<input type="text" size="18" style="width:15%; text-align:center;" value="<fmt:formatDate value="${map.business.startTime}" pattern="yyyy-MM-dd HH:mm" />" readonly>
												至
											    <input type="text" size="18" style="width:15%; text-align:center;" value="<fmt:formatDate value="${map.business.endTime}" pattern="yyyy-MM-dd HH:mm" />" readonly>
											           共
										...
										<tr>
											<td><span>加班事由</span></td>
											<td colspan="20"><textarea rows="7" cols="70" placeholder="加班事由" readonly>${map.business.reason }</textarea></td>
										</tr>
										<%-- <tr>
											<td><span>附件</span></td>
											<td colspan="20">
						...
						<div style="width: 90%; text-align: center;margin:auto;margin-top:5px;">
							...
							<c:if test="${map.isHandler and map.task.name ne '提交申请' and sessionScope.user.id ne '225'}">
								<!-- 当前登录人是管理者  -->
								...
								or sessionScope.user.id eq '28' or sessionScope.user.id eq '30' or sessionScope.user.id eq '463' or sessionScope.user.id eq '484' or sessionScope.user.id eq '129' or sessionScope.user.id eq '6'}">
...
<script type="text/javascript" src="<%=base%>/views/manage/ad/chkatt/overtime/js/process.js"></script>
