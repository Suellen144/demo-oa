...
$(function() {
	dataTable = $("#dataTable").datatable({
		"id": "dataTable",
		"url": web_ctx + "/manage/finance/projectApplication/getAchievementListNew",
		"columns": initColumn(),
		"paging": false,
		"pageSize": 10000,
		"info":false,
...
function getSearchData() {
    var status = "";
	var params = {};
	params.startTime = $.trim($("#startTime").val() + " 00:00:00");
	params.endTime = $.trim($("#endTime").val()+ " 23:59:59");
	if($("#startTime").val() != "" && $("#endTime").val() != "") {
		$("#dataTable2 th:eq(3)").text($.trim($("#startTime").val())+' 至 '+$.trim($("#endTime").val()));
	}else {
		$("#dataTable2 th:eq(3)").text('');
	}
	return params;
...
function rowCallBack(nRow, aData, iDisplayIndex) {
	...
	if(aData.performanceContribution != null){
		$('td:eq(3)', nRow).html(formatCurrency(aData.performanceContribution));
	}
    return nRow;
}
