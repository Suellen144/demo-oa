...
function getNodeHtml() {
	...
	html.push('</select>');
	html.push('</td>');
	html.push('<td><input type="text" name="payCompany" style="width: 100%;border: none;text-align: center;">');
	html.push('<td><input type="text" name="payAccount" style="width: 100%;border: none;text-align: center;">');
	html.push('<td>');
...
function getFormJson() {
	...
	formData["commonReceivedAttachs"] = [];
	...
	$("tbody").find("tr[name='node']").each(function(index, tr) {
		...
		var deptId = $(this).find("input[name='deptsId']").val();
		var payCompany = $(this).find("input[name='payCompany']").val();
		var payAccount = $(this).find("input[name='payAccount']").val();
		var costProperty = $(this).find("select[name='costProperty']").val();
		// 其中一个有值就算有效表单数据
		if(!isNull(id) || !isNull(date) || !isNull(projectManageId)
				|| !isNull(reason) || !isNull(type) 
				|| !isNull(money) || !isNull(payCompany)
				|| !isNull(payAccount) || !isNull(costProperty)) {
			var data = {};
			data["id"] = id;
			data["date"] = date;
			...
			data["costProperty"] = costProperty;
			
			formData["commonReceivedAttachs"].push(data);
		}
	});
...
function checkForm(formData) {
	...
	if(isNull(formData["commonReceivedAttachs"])) {
		checkMsg.push("至少有一条银行常规收款申请！");
		return checkMsg;
	}
	
	$(formData["commonReceivedAttachs"]).each(function(index, data) {
		if(isNull(data["date"])) {
		...
		if(isNull(data["payCompany"])) {
			checkMsg.push("付款单位不能为空！");
			return checkMsg;
		}
		/*if(isNull(data["payAccount"])) {
			checkMsg.push("付款账号不能为空！");
			return checkMsg;
		}*/
	});
	return checkMsg;
...
function deleteAttach(obj) {
	...
	if(!isNull(attachUrl)) {
		bootstrapConfirm("提示", "确定要删除该附件吗？", 300, function() {
			$.ajax({
				url: web_ctx+"/manage/finance/commonReceived/deleteAttach",
...
function initFileUpload() {
	var date = new Date();
	var params = {
		"path": "commonReceive/" + date.getFullYear() + (date.getMonth()+1) + date.getDate(),
...
function del(id) {
	$.ajax({
		url:"/manage/finance/commonReceived/delete",
		type:"post",
		data:{
			"id":id,
		},
