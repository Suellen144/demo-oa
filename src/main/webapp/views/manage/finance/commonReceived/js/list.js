...
$(function() {
	dataTable = $("#dataTable").datatable({
		"id": "dataTable",
		"url": web_ctx + "/manage/finance/commonReceived/getList",
		"columns": initColumn(),
...
function rowCallBack(nRow, aData, iDisplayIndex) {
	...
	if(aData.receivedCompany == "11"){
		$('td:eq(3)', nRow).html("睿哲科技股份有限公司");
	}
	else if(aData.receivedCompany == "1"){
		$('td:eq(3)', nRow).html("新疆睿哲网络科技有限公司");
	}
	else if(aData.receivedCompany == "2"){
		$('td:eq(3)', nRow).html("深圳睿哲网络科技有限公司");
	}
	else if(aData.receivedCompany == "3"){
		$('td:eq(3)', nRow).html("四川睿哲网络科技有限公司");
	}
	else if(aData.receivedCompany == "4"){
		$('td:eq(3)', nRow).html("河南润哲网络科技有限公司");
	}
	else if(aData.receivedCompany == "6"){
		$('td:eq(3)', nRow).html("睿哲科技(香港)办事处");
	}
	else if(aData.receivedCompany == "7"){
		$('td:eq(3)', nRow).html("福建润哲网络科技有限公司");
	}
	else if(aData.receivedCompany == "8"){
		$('td:eq(3)', nRow).html("北京睿哲广联科技有限公司");
	}
	else if(aData.receivedCompany == "9"){
		$('td:eq(3)', nRow).html("睿哲科技(郑州)办事处");
	}
	else if(aData.receivedCompany == "10"){
...
function buildOperate(nRow,aData) {
	var html = [];
	$(nRow).attr("onclick", "location.href='"+web_ctx+"/manage/finance/commonReceived/toAddOrEdit?id="+aData.id+"'");
...
function toAddOrEdit(id){
	window.location.href = web_ctx + "/manage/finance/commonReceived/toAddOrEdit?id="+id;
}

function add(){
	window.location.href = web_ctx + "/manage/finance/commonReceived/toAddOrEdit";
}
