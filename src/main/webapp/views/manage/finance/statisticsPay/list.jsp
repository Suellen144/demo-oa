...
<ul id="myTab" class="nav nav-tabs rlspace">
	<c:choose>
		<c:when test="${sessionScope.user.id eq '27' or sessionScope.user.id eq '225'}">
			<li class="active"><a href="#list" data-toggle="tab">付款统计</a></li>
		</c:when>
		<c:otherwise>
			<li class="active"><a href="#list" data-toggle="tab">付款统计</a></li>
			<li><a href="#normalReceive" data-toggle="tab">收款统计</a></li>
			<li><a href="#normalBargin" data-toggle="tab">其他统计</a></li>
			<li><a href="#normalOther" data-toggle="tab">合同统计</a></li>
		</c:otherwise>
	</c:choose>
</ul>
...
<div class="wrapper">
					...
					<div class="box-header">
							<form id="searchForm" class="form-inline" role="form">
								<div class="col-xs-12">
									...
									<div class="col-xs-4">
										...
										<span>
											<input type="text" name="projectName"  id="projectName" onclick="openProject(this)" style="width:30%;"readonly/>
								...
								<%--	<div class="col-xs-4">
										...
										<span>
											...
											<input type="hidden" id="barginId" name="barginId" value="">
										</span>
									</div>--%>
								</div>
								<br><br>
								<div class="col-xs-12">
									<div class="col-xs-4">
										<span><label class="control-label">总经理审批时间：</label></span>
										<input id="zBeginDate" name="zBeginDate" placeholder="开始时间" style="background-color: inherit;width: 30%;" readonly>
										<span>到</span>
										<input id="zEndDate" name="zEndDate" placeholder="结束时间" style="background-color: inherit;width: 30%;" readonly>
									</div>
									<div class="col-xs-4">
										<span><label class="control-label">出纳审批时间：</label></span>
										<input id="cBeginDate" name="cBeginDate" placeholder="开始时间" style="background-color: inherit;width: 30%;" readonly>
										<span>到</span>
										<input id="cEndDate" name="cEndDate" placeholder="结束时间" style="background-color: inherit;width: 30%;" readonly>
									</div>
								...
								<div class="col-xs-12">
									<div class="col-xs-4">
										<c:choose>
											...
											<span>
												<input type="text" id="deptName" name="deptName" value="市场部" readonly/>
												<input type="text" name="deptId" id="deptId" value="3" style="display:none;">
											</span>
											</c:when>
											<c:otherwise>
												<span><label class="control-label">部门：</label></span>
									...
									<div class="col-xs-4">
										<span><label class="control-label">类别：</label></span>
										<span><select name="generalReimbursType" id="generalReimbursType">
												<option value="">全选</option>
								...
								<div class="col-xs-12">
									<div class="col-xs-4">
										<c:if test="${sessionScope.user.id ne '225'}">
									...
									<div class="col-xs-4">
										...
										<select id="status" name="status" class="form-control">
											<option value=""></option>
											<option value="10">总经理审批（汇总）</option>
						...
						<br><br>
							<div style="text-align:center;">
								...
								<c:choose>
									<c:when test="${sessionScope.user.id eq '27' or sessionScope.user.id eq '225'}">
										<button type="button" class="btn btn-default" onclick="clearForm2()" style="margin-left:10px;">清空</button>
									</c:when>
									<c:otherwise>
								 ...
								 <button type="button"
										class="btn btn-primary  fa fa-x fa-cloud-download"
											onclick="exportExcel()" style="margin-left:10px;" title="统计"></button> 
								<button type="button"
										class="btn btn-primary  fa fa-x fa-cloud-download"
											onclick="exportExcel1()" style="margin-left:10px;"  title="详细"></button>
							</div>
					</div>
				</div>
			</div>
	...
	<div class="tab-pane fade" id="normalReceive">
		<iframe id="normalReceive_iframe" name="normalReceive_iframe" width="100%" style="min-height: 768px"
				frameborder="no" scrolling="yes" src="<%=base%>/manage/finance/statisticsReceived/toList"></iframe>
	</div>
	<div class="tab-pane fade" id="normalBargin">
		<iframe id="normalBargin_iframe" name="normalReceive_iframe" width="100%" style="min-height: 768px"
				frameborder="no" scrolling="yes" src="<%=base%>/manage/finance/statisticsBargin/toList"></iframe>
	</div>
	<div class="tab-pane fade" id="normalOther">
		<iframe id="normalOther_iframe" name="normalOther_iframe" width="100%" style="min-height: 768px"
				frameborder="no" scrolling="yes" src="<%=base%>/manage/finance/statisticsBargin/toOtherList"></iframe>
	</div>
</div>
...
<script type="text/javascript" src="<%=base%>/views/manage/finance/statisticsPay/js/list.js"></script>
...
<script>
window.document.getElementById("normalOther_iframe").contentWindow.onscroll = function aaa() {
    var ifm = document.getElementById("normalOther_iframe").contentWindow.document.documentElement;
    var normalOther_iframe = window.frames["normalOther_iframe"].document.getElementById("reimburse_header");
    var top = normalOther_iframe.clientHeight;
    
    if(top < ifm.scrollTop){ 
    	console.log(normalOther_iframe)
    	normalOther_iframe.classList.add("topIfm")
    }else{
    	normalOther_iframe.classList.remove("topIfm")
    }
}
