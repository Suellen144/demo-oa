...
function initCkeditor() {
	...
	var height = bodyHeight * 0.5; // 百分之五十
...
function getDeptList() {
	$.ajax({
		url: web_ctx+"/manage/sys/dept/getDeptList",
		type: "get",
		dataType: "json",
		async: false,
		success: function(data) {
			recursDept(deptList, data);
...
function recursDept(deptList, data) {
	$(data.children).each(function(index, child) {
		deptList.push(child);
		recursDept(deptList, child);
	});
}
...
function submitForm() {
	var formDate = $("#form1").serializeJson();
	if($("#approveStatus").val() == 1) {
		if( !isNull(formDate.publishTime) ) {
			
			if( !dateRep.test(formDate.publishTime) ) {
				var publishTime = new Date(formDate.publishTime).pattern("yyyy-MM-dd HH:mm");
			} else {
				var publishTime = new Date(Date.parse(formDate.publishTime.replace(/-/g,"/"))).pattern("yyyy-MM-dd HH:mm");
			}
		/*	var publishTime = new Date(formDate.publishTime).pattern("yyyy-MM-dd HH:mm");*/
			var now = new Date();
			if( publishTime < now ) {
				formDate.actualPublishTime = now.pattern("yyyy-MM-dd HH:mm");
			} else {
				formDate.actualPublishTime = publishTime;
			}
		} else {
			formDate.actualPublishTime = new Date().pattern("yyyy-MM-dd HH:mm");
		}
	}
	
	$.ajax({
		url: web_ctx+"/manage/office/noitce/update",
		type: "post",
		data: formDate,
		dataType: "json",
		success: function(data) {
...
function submitProcess() {
	var variables = {};
	var compResult = completeTask($("#taskId").val(), variables);

	if(compResult.code == 1) {
		$.ajax({
			url: web_ctx+"/manage/office/noitce/setApproveStatus",
			type: "post",
			data: {"id": $("#id").val(), "approveStatus": $("#approveStatus").val()},
			dataType: "json",
			success: function(data) {
				openBootstrapShade(false);
...
//	$("#modal_content").html(content);
	...
	$("#modal_type").val(formData.type == "1" ? "通知" : "提示");
	...
	if(isNull(formData.deptName)) {
		...
		$("#modal_deptName").text(deptName.join(","));
	}
	else{
