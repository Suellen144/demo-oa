$(function() {
	 ...
	 $("#table3").datatable({
		"id": "table3",
		"url": web_ctx + "/manage/finance/pay/getListNew",
		"columns": initColumn(),
		"paging": false,
		"pageSize": 1000,
		"info":false,
...
/*	if(iDisplayIndex % 2 == 0) {
	...
	if(aData.actualPayDate != null && aData.actualPayDate != ''){
		$('td:eq(0)', nRow).html(dateFormatRefactoring(aData.actualPayDate));
	}
	if(aData.sysUser != null){
		$('td:eq(1)', nRow).html(aData.sysUser.name);
	}else{
		$('td:eq(1)', nRow).html("");
	}
	if(aData.actualPayMoney != null){
		$('td:eq(2)', nRow).html(initInputMask(aData.actualPayMoney));
	}else{
		$('td:eq(2)', nRow).html("");
	}
	if(aData.status == "1") {
		$('td:eq(4)', nRow).html('部门经理审批');
	}else if(aData.status == "11"){
		$('td:eq(4)', nRow).html('项目负责人审批');
	} else if(aData.status == "2") {
		$('td:eq(4)', nRow).html('财务审批');
	} else if(aData.status == "3") {
		$('td:eq(4)', nRow).html('总经理审批');
	} else if(aData.status == "4") {
		$('td:eq(4)', nRow).html('出纳审批');
	}else if(aData.status == "5") {
		$('td:eq(4)', nRow).html('已归档');
	} else if(aData.status == "6") {
		$('td:eq(4)', nRow).html('取消申请');
	} else if(aData.status == "7" || aData.status == "8" || aData.status == "9" || aData.status == "10") {
		$('td:eq(4)', nRow).html('提交申请');
...
function buildOperate(nRow,aData,type) {
	...
	if(type == 0){
		...
		//收票跳转
		$(nRow).attr("onclick", "location.href='"+web_ctx+"/manage/sale/ticketConfirmation/toAddOrEdit?barginManageId="+$("#id").val()+"&id="+aData.id+"'");
	}
	//$(nRow).attr("onclick", "toProcess("+aData.processInstanceId+")");
...
function toAddOrEdit(){
	...
	window.location.href = web_ctx + "/manage/sale/ticketConfirmation/toAddOrEdit?barginManageId="+barginManageId;
}

function toPayAdd(){
	var barginManageId=$("#id").val();
	window.location.href = web_ctx + "/manage/finance/pay/toAddOrEdit?barginManageId="+barginManageId;
}
/******************************
 * 		全局变量			  	  *
 ******************************/
