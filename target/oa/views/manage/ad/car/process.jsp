				...
				<div class="box box-primary tbspace">
					...
					<%-- 	<input type="hidden" id="attachments" name="attachments" value="${map.business.attachments }">
						...
						<table id="table1">
									...
									<thead>
										<c:choose>
											<c:when test="${map.business.status eq '4' }">
												<tr><th colspan="20">用车详情表</th></tr>
											</c:when>
											<c:otherwise>
												<tr><th colspan="20">用车申请表</th></tr>
											</c:otherwise>
										</c:choose>
									</thead>
									<tbody>
										...
										<tr>
											...
											<td><span>目的地</span></td>
											<td>
												<input name="desitination" type="text" value="${map.business.desitination}">
											</td>
										</tr>
										<tr>
											<td><span>用车时间</span></td>
											<td colspan="20" style="padding-right:30px;">
												自
												<input id="startTime" name="startTime" size="18" class="starttime" style="width:26%;" value="<fmt:formatDate value="${map.business.startTime}" pattern="yyyy-MM-dd HH:mm" />" readonly>
												至
											    <input id="endTime" name="endTime" size="18" class="endtime" style="width:26%;" value="<fmt:formatDate value="${map.business.endTime}" pattern="yyyy-MM-dd HH:mm" />" readonly>
											</td>
										</tr>
										<tr>
											<td><span>用车事由</span></td>
											<td colspan="20"><textarea name="reason" rows="7" cols="70" placeholder="用车事由">${map.business.reason }</textarea></td>
										</tr>
									<%-- 	<tr>
											<td><span>附件</span></td>
								...
								<c:otherwise>
									...
									<tbody>
										...
										<tr>
											...
											<td><span>目的地</span></td>
											<td>
												<input name="desitination" type="text" value="${map.business.desitination}" readonly>
											</td>
										</tr>
										<tr>
											<td><span>用车时间</span></td>
							...
							<tfoot>
								<c:if test="${(map.isHandler and map.task.name ne '提交申请') or (map.business.status eq 3 and sessionScope.user.id eq 101)}">
									<tr>
										<td colspan="34">
						...
						<div style="width: 90%; text-align: center;margin:auto;margin-top:5px;">
							...
							<c:if test="${map.isHandler and map.task.name ne '提交申请' and map.task.name ne '司机' and sessionScope.user.id ne '225'}">
							...
							<c:if test="${(map.isHandler and map.task.name ne '提交申请' and map.task.name eq '司机') or (map.business.status eq 3 and sessionScope.user.id eq 101) }">
								<button type="button" class="btn btn-primary" onclick="approve('确认')">确认</button>
...
<script type="text/javascript" src="<%=base%>/views/manage/ad/car/js/process.js"></script>
