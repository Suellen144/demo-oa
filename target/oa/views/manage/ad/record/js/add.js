...
function initDeptSelect() {
	var dataTR="";
	$.ajax({
		url: web_ctx+"/manage/ad/record/findPostAppointmentDeptByDeptId",
		type: "post",
		async: false,
		...
		success: function(data) {
			$.each(data,function(key,val){
				dataTR +='<option value="'+data[key].id+'">'+data[key].name+'</option>';
			})
			$("select[name=postAppointmentDept]").append(dataTR);
		},
		error: function(data) {
...
//根据部门生成项目组下拉框
function onPostAppointmentProjectTeam() {
	$("#postAppointmentProjectTeam").find("option").remove();
	var deptValue = $('#postAppointmentDept option:selected') .val();
	if(deptValue!= "" && !isNull(deptValue)) {
		var dataTR = "";
		$.ajax({
			url: web_ctx + "/manage/ad/record/findProjectTeamByDeptId",
			type: "post",
			async: false,
			data: {"deptValue": deptValue},
			dataType: "json",
			success: function (data) {
				dataTR += '<option value=""></option>';
				$.each(data, function (key, val) {
					dataTR += '<option value="' + data[key].id + '">' + data[key].name + '</option>';
				})
				$("select[name=postAppointmentProjectTeam]").append(dataTR);
			},
			error: function (data) {
				bootstrapAlert("提示", "网络错误，请稍后重试！", 400, null);
			}
...
function initTd(){
	...
	if(key==""||key==null){
		$("tr[name='salary']").each(function(index,tr){
			$(tr).find("input[name='basePay'],input[name='meritPay'],input[name='agePay'],input[name='lunchSubsidy'],input[name='computerSubsidy'],input[name='accumulationFund']").attr('readonly', true);
		})
		...
		$("tr[name='salary']").each(function(){
			$(tr).find("input[name='basePay'],input[name='meritPay'],input[name='agePay'],input[name='lunchSubsidy'],input[name='computerSubsidy'],input[name='accumulationFund']").removeAttr('readonly');
		})
...
function savedata() {
	...
	else if($("#form").valid() && !checkAccount()){
		$.ajax({
			url:web_ctx+"/manage/ad/record/addData",
			type: "POST",
			...
			success:function(data){
				if(data.code==1){
					...
					bootstrapAlert("提示", "薪酬更改操作，请先导入密钥！", 400, null);
				}
				 else if(data.code==3) {
						bootstrapAlert("提示", "密钥错误，请导入正确密钥！", 400, null);
					}
				 else{
					 bootstrapAlert("提示", "保存失败！", 400, null);
				 }
...
function getSameData(){
	...
	if($(tr).find("select[name='postAppointmentDept'] option:selected").text()!=""){
		$("#table1").find("input[name='dept']").val($(tr).find("select[name='postAppointmentDept'] option:selected").text());
	...
	if($(tr).find("select[name='postAppointmentCompany'] option:selected").text()!=""){
		$("#table1").find("input[name='company']").val($(tr).find("select[name='postAppointmentCompany'] option:selected").text());
	}
	if($(tr).find("select[name='postAppointmentProjectTeam'] option:selected").text()!=""){
		$("#table1").find("input[name='projectTeam']").val($(tr).find("select[name='postAppointmentProjectTeam'] option:selected").text());
	}
	if($(tr).find("select[name='station'] option:selected").text()!=""){
		$("#table1").find("input[name='position']").val($(tr).find("select[name='station'] option:selected").text());
	...
	if($(tr1).find("input[name='educationMajor']").val()!=""){
		$("#table1").find("input[name='major']").val($(tr1).find("input[name='educationMajor']").val());
	}
	if($(tr1).find("input[name='educationSchool']").val()!=""){
		$("#table1").find("input[name='school']").val($(tr1).find("input[name='educationSchool']").val());
	}
	
	if($(tr1).find("select[name='educationEducation'] option:selected").text()!=""){
		$("#table1").find("input[name='education']").val($(tr1).find("select[name='educationEducation'] option:selected").text());
	}
	/*var tr2=findSignLastData();
	if($(tr2).find("input[name='beginDate']").val()!=""){
		$("#table1").find("input[name='entryTime']").val($(tr2).find("input[name='beginDate']").val());
	}*/
}
...
function makeValidRule() {
    $("#form").validate({
        rules: {
            ...
            workBeginDate: {required: true},
        },
        messages: {
            entryTime: {required: "入职时间不能为空"},
            workBeginDate: {required: "合同开始时间不能为空"},
        },
		...
		showErrors: function(errorMap, errorList) { }// 覆盖默认error显示
    });
}
...
function checkForm(formData) {
	var text = [];
	if( !isNull(formData["emergencyPhone"]) && !emergencyPhone.test(formData["emergencyPhone"]) ) {
	...
	$(formData["arbeitsvertrags"]).each(function(index, arbeitsvertrag) {
			if(arbeitsvertrag["barginType"]!="0"&&isNull(arbeitsvertrag["endDate"])){
				text.push("请填写非劳动合同的结束时间！");
			}else if(isNull(arbeitsvertrag["signDate"])||isNull(arbeitsvertrag["company"])||isNull(arbeitsvertrag["beginDate"])||isNull(arbeitsvertrag["endDate"])
					||isNull(arbeitsvertrag["barginType"])){
				text.push("请将劳动合同填写完全！");
			}else if(!checkDate(arbeitsvertrag["beginDate"],arbeitsvertrag["endDate"])){
				text.push("劳动合同开始时间不能大于结束时间！");
			}
	});
	
	$(formData["payAdjustments"]).each(function(index, payAdjustment) {
		if(isNull(payAdjustment["changeDate"])||isNull(payAdjustment["basePay"])||isNull(payAdjustment["meritPay"])||isNull(payAdjustment["total"])){
			text.push("请将薪酬调整记录填写完全！");
		}
	
	});
	
	$(formData["postAppointments"]).each(function(index, postAppointment) {
		if(isNull(postAppointment["postDate"])||isNull(postAppointment["company"])||isNull(postAppointment["dept"])||
				isNull(postAppointment["station"])||isNull(postAppointment["appoint"])){
			text.push("请将岗位任免填写完全！");
		}
	
	});
	
	$(formData["jobRecords"]).each(function(index, jobRecord) {
		if(isNull(jobRecord["beginDate"])||isNull(jobRecord["endDate"])||isNull(jobRecord["company"])||isNull(jobRecord["station"])){
			text.push("请将以往工作记录填写完全！");
		}else if(!checkDate(jobRecord["beginDate"],jobRecord["endDate"])){
			text.push("以往工作记录开始时间不能大于结束时间！");
		}
	
	});
	
	$(formData["educations"]).each(function(index, education) {
		if(isNull(education["beginDate"])||isNull(education["endDate"])||isNull(education["school"])||
		   isNull(education["department"])||isNull(education["major"])||isNull(education["education"])){
			text.push("请将教育背景填写完全！");
		}else if(!checkDate(education["beginDate"],education["endDate"])){
			text.push("教育背景开始时间不能大于结束时间！");
		}
	
	});
	
	$(formData["certificates"]).each(function(index, certificate) {
		if(isNull(certificate["date"])||isNull(certificate["issuingUnit"])||isNull(certificate["scannings"])
...
$ ("#dept").parent("td").hover (function (e) {
	$.ajax({
		url:web_ctx+"/manage/ad/record/showDeptHistory",
		type: "POST",
		dataType: "json",
		...
		success:function(data){
			if(data.code==1){
				var html = [];
				html.push('<tr><th colspan="10" style="text-align:left;font-size: 14px;padding: 4px;background-color:#FFF;">历史部门记录</th></tr>');
				html.push("<tr style='font-size: 12px;padding: 4px;'>");
				html.push("<td style='width:20%;padding: 4px;background-color:#DDDDDD'>任命时间</td>");
				html.push("<td style='width:20%;padding: 4px;background-color:#DDDDDD'>撤销时间</td>");
				html.push("<td style='width:40%;padding: 4px;background-color:#DDDDDD'>部门</td>");
				html.push("<td style='width:20%;padding: 4px;background-color:#DDDDDD'>备注</td>");
				html.push("</tr>");

				var depts = data.result;
				for (var i = 0; i < depts.length; i++) {
					var remark = isNull(depts[i].remark) ? " " :depts[i].remark ;
					var endTime = isNull(depts[i].endTime) ? "  " : new Date(depts[i].endTime).pattern("yyyy-MM-dd");
					html.push("<tr style='font-size: 12px;padding: 4px;background-color:#FFF;'>");
					html.push("<td style='padding: 4px;background-color:#FFF;'>"+new Date(depts[i].startTime).pattern("yyyy-MM-dd")+"</td>");
					html.push("<td style='padding: 4px;background-color:#FFF;'>"+endTime+"</td>");
					html.push("<td style='padding: 4px;background-color:#FFF;'>"+depts[i].dept+"</td>");
					html.push("<td style='text-align:left;height:auto;white-space:pre-line;width:auto;word-break:break-all;padding: 4px;background-color:#FFF;'>"+remark+"</td>");
					html.push("</tr>");
				}
				$("#dept-box-body").html(html);
				$("#dept-div").css({"position":"absolute","left" : e.clientX + "px","top" : e.clientY + "px",opacity:"show"});
				$("#dept-div").show();
			}
		},error:function(data) {
			bootstrapAlert("提示", "网络错误，请稍后重试！", 400, null);
	...
	$("#dept-div").css({left:0,top:0,opacity:"hide"});
	$("#dept-div").hide();
});
...
$("#position").parent("td").hover(function(e){
	$.ajax({
		url:web_ctx+"/manage/ad/record/showPositionHistory",
		type: "POST",
		dataType: "json",
		data:{"userId":$("#userId").val()},
		success:function(data){
			if(data.code==1){
				var html = [];
				html.push('<tr><th colspan="10" style="text-align:left;font-size: 14px;padding: 4px;background-color:#FFF;">历史岗位记录</th></tr>');
				html.push("<tr  style='font-size: 12px;padding: 4px;'>");
				html.push("<td style='width:20%;padding: 4px;background-color:#DDDDDD;'>任命时间</td>");
				html.push("<td style='width:20%;padding: 4px;background-color:#DDDDDD;'>撤销时间</td>");
				html.push("<td style='width:40%;padding: 4px;background-color:#DDDDDD;'>岗位</td>");
				html.push("<td style='width:20%;padding: 4px;background-color:#DDDDDD;'>备注</td>");
				html.push("</tr>");

				var positions = data.result;
				for (var i = 0; i < positions.length; i++) {
					var remark = isNull(positions[i].remark) ? " " :positions[i].remark ;
					var endTime = isNull(positions[i].endTime) ? "  " : new Date(positions[i].endTime).pattern("yyyy-MM-dd");
					html.push("<tr>");
					html.push("<td style='padding: 4px;background-color:#FFF;'>"+new Date(positions[i].startTime).pattern("yyyy-MM-dd")+"</td>");
					html.push("<td style='padding: 4px;background-color:#FFF;'>"+endTime+"</td>");
					html.push("<td style='padding: 4px;text-align:left;background-color:#FFF;'>"+positions[i].position+"</td>");
					html.push("<td style='text-align:left;height:auto;white-space:pre-line;width:auto;word-break:break-all;padding: 4px;background-color:#FFF;'>"+remark+"</td>");
					html.push("</tr>");
				}
				$("#position-box-body").html(html);
				$("#position-div").css({"position":"absolute","left":e.clientX,"top":e.clientY,opacity:"show"});
				$("#position-div").show();
			}
		},
		error:function(data) {
	...
	$("#position-div").css({left:0,top:0,opacity:"hide"});
	$("#position-div").hide();
});
...
function addDeptHistory(){
	...
	window.location.href=web_ctx+"/manage/ad/record/addDeptHistory?userId="+userId;
}

function addPositionHistory(){
	var  userId = $("#userId").val();
	window.location.href=web_ctx+"/manage/ad/record/addPositionHistory?userId="+userId;
}

function addSalaryHistory(){
	var  userId = $("#userId").val();
	window.location.href=web_ctx+"/manage/ad/record/addSalaryHistory?userId="+userId;
}
...
function initInputMask() {
	...
	$("#emergencyPhone").inputmask({ "mask": "9{11}" });
	$("#idcard").inputmask({ "mask": "*{18}", "placeholder": "" });
 //   $("input[name='basePay'],input[name='meritPay']").inputmask("Regex", { regex: "\\d+\\.?\\d{0,2}" });
    $("input[name='agePay'],input[name='lunchSubsidy']").inputmask("Regex", { regex: "\\d+\\.?\\d{0,2}" });
    $("input[name='computerSubsidy'],input[name='accumulationFund']").inputmask("Regex", { regex: "\\d+\\.?\\d{0,2}" });
}
...
function initDatetimepicker() {
    ...
    $(".changeDate,.postDate,.signDate,.beginDate,.endDate,.date").datetimepicker({
        minView: "month",
        language:"zh-CN",
        format: "yyyy-mm-dd",
        toDay: true,
	...
	$(".validity").datetimepicker({
		...
		$(this).find("select[name='isValidity']").val("1");
	})
...
function setNode(tr) {
    ...
    $(tr).find("input[name='basePay'],input[name='meritPay']," +
        "input[name='agePay'],input[name='lunchSubsidy'],"+
		"input[name='computerSubsidy'],"+
		"input[name='accumulationFund']").bind("keyup", function() {
        updateTotal(); // 更新总计数据
    });
...
function initInputKeyUp() {
		 $("input[name='basePay'],input[name='meritPay']," + "input[name='agePay'],input[name='lunchSubsidy'],"+ "input[name='computerSubsidy'],"+ "input[name='accumulationFund']").bind("keyup", function() {
		        updateTotal(); // 更新总计数据
		    });
		...
		$(indexTr).find("input[name='scanFile']").fileupload(
				{
					...
					messages : {
						...
						if (result.execResult.code != 0) {
							...
							$(indexTr).find("input[name='hiddenPath']").val(result.path);
							$(indexTr).find("input[name='hiddenName']").val(result.originName);
						} else {
...
function getHtmlForSalaryRecord(){
    ...
    html.push('	<td style="width: 6%;"><input type="text"  name="basePay"></td>');
    html.push('	<td style="width: 6%;"><input type="text"  name="meritPay"></td>');
    html.push('	<td style="width: 6%;"><input type="text" name="agePay"></td>');
    html.push('	<td style="width: 6%;"><input type="text" name="lunchSubsidy"></td>');
    html.push('	<td style="width: 5%;"><input type="text" style="text-align:right;"   name="computerSubsidy"></td>');
    html.push('	<td style="width: 5%;"><input type="text"  style="text-align:right;"  name="accumulationFund"></td>');
    html.push('	<td style="width: 5%;"><div style="display:flex"><span  id="total"  name="total"></span> </div></td>');
    html.push('	<td style="width: 18%;"><textarea  name="payReason"></textarea></td>');
    //html.push('	<td style="width: 4%;border-right-style:hidden;"><span style="margin-left: 20%"><a href="javascript:void(0);" style="font-size:x-large;" onclick="node(\'add\', \'salaryRecord\', this)"><img alt="添加" src="'+base+'/static/images/add.png" style="margin-right: 6px"></a> <a href="javascript:void(0);" style="font-size:x-large;" onclick="node(\'del\', \'salaryRecord\', this)"><img alt="删除" src="'+base+'/static/images/del.png" ></a></span></td>');
    html.push('</tr>');
    return html.join("");
}
...
function getHtmlForStationRecord(){
    ...
    html.push('	<td style="width: 10%;"><select style="width:100%;" name="station"> <option value=""></option>'+$("#station_hidden").html()+'</select></td>');
    html.push('	<td style="width: 3%;"><select style="width:100%;" name="appoint"> <option value=""></option>'+$("#appoint_hidden").html()+'</select></td>');
    html.push('	<td style="width: 18%;"><textarea  name="postReason"></textarea></td>');
   // html.push('	<td style="width: 4%;border-right-style:hidden;"><span style="margin-left: 20%"><a href="javascript:void(0);" style="font-size:x-large;" onclick="node(\'add\', \'stationRecord\', this)"><img alt="添加" src="'+base+'/static/images/add.png" style="margin-right: 6px"></a> <a href="javascript:void(0);" style="font-size:x-large;" onclick="node(\'del\', \'stationRecord\', this)"><img alt="删除" src="'+base+'/static/images/del.png" ></a></span></td>');
    html.push('</tr>');
...
function getHtmlForSignRecord(){
    ...
    html.push('	<td style="width: 18%;"><textarea  name="signReason"></textarea></td>');
    //html.push('	<td style="width: 4%;border-right-style:hidden;"><span style="margin-left: 20%"><a href="javascript:void(0);" style="font-size:x-large;" onclick="node(\'add\', \'signRecord\', this)"><img alt="添加" src="'+base+'/static/images/add.png" style="margin-right: 6px"></a> <a href="javascript:void(0);" style="font-size:x-large;" onclick="node(\'del\', \'signRecord\', this)"><img alt="删除" src="'+base+'/static/images/del.png" ></a></span></td>');
    html.push('</tr>');
...
function getHtmlForOldworkRecord(){
    ...
    html.push('	<td style="width: 27.2%;"><input type="text" name="jobRecordCompany"></td>');
    html.push('	<td style="width: 5%;"><input type="text" name="station"></td>');
    html.push('	<td style="width: 18%;"><textarea  name="duty"></textarea></td>');
   // html.push('	<td style="width: 4%;border-right-style:hidden;"><span style="margin-left: 20%"><a href="javascript:void(0);" style="font-size:x-large;" onclick="node(\'add\', \'oldworkRecord\', this)"><img alt="添加" src="'+base+'/static/images/add.png" style="margin-right: 6px"></a> <a href="javascript:void(0);" style="font-size:x-large;" onclick="node(\'del\', \'oldworkRecord\', this)"><img alt="删除" src="'+base+'/static/images/del.png" ></a></span></td>');
    html.push('</tr>');
...
function getHtmlForEducationRecord(){
    ...
    html.push('	<td style="width: 6.8%;border-left-style:hidden;"><input type="text" name="beginDate" class="beginDate" readonly></td>');
    html.push('	<td style="width: 6.8%;"><input type="text" name="endDate" class="endDate" readonly></td>');
    html.push('	<td style="width: 12.2%;"><input type="text" name="educationSchool"></td>');
    html.push('	<td style="width: 20%;"><input type="text" name="department"></td>');
    html.push('	<td style="width: 9%;"><input type="text" name="educationMajor"></td>');
    ...
    //html.push('	<td style="width: 4%;border-right-style:hidden;"><span style="margin-left: 20%"><a href="javascript:void(0);" style="font-size:x-large;" onclick="node(\'add\', \'educationRecord\', this)"><img alt="添加" src="'+base+'/static/images/add.png" style="margin-right: 6px"></a> <a href="javascript:void(0);" style="font-size:x-large;" onclick="node(\'del\', \'educationRecord\', this)"><img alt="删除" src="'+base+'/static/images/del.png" ></a></span></td>');
    html.push('</tr>');
...
function getHtmlForHonorRecord(){
    ...
    html.push('	<td style="width: 8%;"><input type="text"  class="date" name="date" readonly></td>');
    html.push('	<td style="width: 10%;"><input type="text" name="issuingUnit"></td>');
    ...
    html.push('	<td style="width: 9%;"><select name="isValidity" onchange="selectOnchang(this)" style="width:98%;"><option value=""></option><option value="0">长期</option><option value="1">否</option></select></td>');
    html.push('	<td style="width: 20%;"> <input type="text"  name="scanName"  value="" readonly onclick="selectScan(this)" placeholder="选择扫描件"> <input type="file"  name="scanFile" style="display:none;"></td>');
   // html.push('	<td style="width: 4%;border-right-style:hidden;"><span style="margin-left: 20%"><a href="javascript:void(0);" style="font-size:x-large;" onclick="node(\'add\', \'educationRecord\', this)"><img alt="添加" src="'+base+'/static/images/add.png" style="margin-right: 6px"></a> <a href="javascript:void(0);" style="font-size:x-large;" onclick="node(\'del\', \'educationRecord\', this)"><img alt="删除" src="'+base+'/static/images/del.png" ></a></span></td>');
    html.push('</tr>');
...
function getFormData() {
	...
	$("tbody").find("tr[name='salary']").each(
		function (index,tr) {
			var id=$(this).find("input[name='payAdjustmentId']").val();
			var recordId=$("#id").val();
			var changeDate = $(this).find("input[name='changeDate']").val();
			var basePay = $(this).find("input[name='basePay']").val();
			var meritPay = $(this).find("input[name='meritPay']").val();
			var agePay = $(this).find("input[name='agePay']").val();
			var lunchSubsidy = $(this).find("input[name='lunchSubsidy']").val();
			var computerSubsidy = $(this).find("input[name='computerSubsidy']").val();
			var accumulationFund = $(this).find("input[name='accumulationFund']").val();
            var payReason = $(this).find("textarea[name='payReason']").val();
            var total=$(this).find("span[name='total']").text();
            if(!isNull(total)||!isNull(changeDate)||!isNull(basePay)||!isNull(meritPay)){
            	var salaryRecord = {};
            	salaryRecord["id"] = id;
    ...
    $("tbody").find("tr[name='station']").each(
        function (index,tr) {
            ...
            var company = $(this).find("select[name='postAppointmentCompany']").val();
            var dept = $(this).find("select[name='postAppointmentDept']").val();
            var projectTeam = $(this).find("select[name='postAppointmentProjectTeam']").val();
            var station = $(this).find("select[name='station']").val();
            var appoint = $(this).find("select[name='appoint']").val();
            var postReason = $(this).find("textarea[name='postReason']").val();

            if(!isNull(postDate)||!isNull(company)||!isNull(dept)||!isNull(projectTeam)||!isNull(station)||!isNull(appoint)){
                var stationRecord = {};
                stationRecord["id"] = id;
    ...
    $("tbody").find("tr[name='sign']").each(
        function (index,tr) {
            ...
            var company = $(this).find("select[name='arbeitsvertragCompany']").val();
            var beginDate = $(this).find("input[name='beginDate']").val();
            var endDate = $(this).find("input[name='endDate']").val();
            var barginType = $(this).find("select[name='barginType']").val();
            var signReason = $(this).find("textarea[name='signReason']").val();

            if(!isNull(signDate)||!isNull(company)||!isNull(beginDate)||!isNull(endDate)||!isNull(barginType)){
                var signRecord = {};
                signRecord["id"] = id;
    ...
    $("tbody").find("tr[name='oldwork']").each(
        function (index,tr) {
            ...
            var beginDate = $(this).find("input[name='beginDate']").val();
            var endDate = $(this).find("input[name='endDate']").val();
            var company = $(this).find("input[name='jobRecordCompany']").val();
            var station = $(this).find("input[name='station']").val();
            var duty = $(this).find("textarea[name='duty']").val();

            if(!isNull(beginDate)||!isNull(endDate)||!isNull(station)||!isNull(company)){
                var oldworkRecord = {};
                oldworkRecord["id"] = id;
    ...
    $("tbody").find("tr[name='education']").each(
        function (index,tr) {
        	var id=$(this).find("input[name='educationId']").val();
			var recordId=$("#id").val();
            var beginDate = $(this).find("input[name='beginDate']").val();
            var endDate = $(this).find("input[name='endDate']").val();
            var school = $(this).find("input[name='educationSchool']").val();
            var department = $(this).find("input[name='department']").val();
            var major = $(this).find("input[name='educationMajor']").val();
            var education = $(this).find("select[name='educationEducation']").val();

            if(!isNull(beginDate)||!isNull(endDate)||!isNull(school)||!isNull(department)||!isNull(major)||!isNull(education)){
                var educationRecord = {};
    ...
    $("tbody").find("tr[name='honor']").each(
        function (index,tr) {
        	var id=$(this).find("input[name='certificateId']").val();
        	var recordId=$("#id").val();
            var date = $(this).find("input[name='date']").val();
            var issuingUnit = $(this).find("input[name='issuingUnit']").val();
            ...
            var hiddenPath = $(this).find("input[name='hiddenPath']").val();
            var hiddenName = $(this).find("input[name='hiddenName']").val();
            if(!isNull(date)||!isNull(issuingUnit)||!isNull(hiddenPath)||!isNull(hiddenName)){
                var honorRecord = {};
                honorRecord["id"] = id;
...
function setUserName(obj){
	...
	var userPinYin = Pinyin.GetHP($(obj).val());
	userPinYin += "@reyzar.com";
	judgeEmail(userPinYin);
	if(isNull($(obj).val()) || $(obj).val() == "") {
		$("#email,#account").val("");
	}
}

//根据用户名字拼音自动生成工作邮箱,账号
function judgeEmail(userPinYin) {
	$.ajax({
		url: web_ctx+"/manage/ad/record/judgeEmail",
		type: "post",
		async: false,
		data: {"email": userPinYin},
		dataType: "json",
		success: function(data) {
			if(data.code == 1) {
				$("#email").val(userPinYin);
				$("#account").val(Pinyin.GetHP($("#name").val()));
			} else {
				var userEmail = Pinyin.GetQP($("#name").val());
                userEmail += "@reyzar.com";
				$.ajax({
					url: web_ctx+"/manage/ad/record/judgeEmail2",
					type: "post",
					async: false,
					...
					success: function(data) {
						if(data.code == 1) {
							var number = data.result+1;
							var userEmail2 = Pinyin.GetQP($("#name").val()) + number + "@reyzar.com";
							$("#email").val(userEmail2);
                            $("#account").val(Pinyin.GetQP($("#name").val()) +number);
						} else {
							$("#email").val(userEmail);
                            $("#account").val(Pinyin.GetQP($("#name").val()));
						}
					},
					error: function(data) {
						bootstrapAlert("提示", "网络错误，请稍后重试！", 400, null);
...
function checkAccount() {
	...
	var flag = true;
	$.ajax({
		url: web_ctx+"/manage/sys/user/checkAccount",
		type: "post",
		async: false,
